...
//         setGraphicsLcdColumn(0);
//         for (x = 0; x < 128; x++)
//             sendGraphicsLcdData(epd_bitmap_mockup[i++]);
//     }
// }
...
void initGraphicsLcd(DisplayContext * displayContext)
{
    ...
    GPIO_PORTD_PUR_R |= CLK_MASK;                      // pullup must be enabled when SPO=1 (see 15.4)

    // Configure the SSI1 as a SPI master, mode (0,0), 8bit operation, 1 MHz bit rate
    ...
    SSI1_CR0_R = SSI_CR0_SPH | SSI_CR0_SPO | SSI_CR0_FRF_MOTO | SSI_CR0_DSS_8;
